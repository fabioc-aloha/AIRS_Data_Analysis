...
- **Decision Dashboard (FACT_CHECK_AUDIT.md)**: D1–D11 all resolved (k=3 clusters adopted, SRMR fixed, R² corrected, CI fabrication fixed, discriminant validity claims corrected, Trust claims reframed, p=.009 → .007 fixed, “student” → “academic” label fixed).
...
- **Thesis build**: `thesis-v2/build.ps1` (Pandoc + XeLaTeX + Mermaid CLI). Decision Dashboard in `FACT_CHECK_AUDIT.md` confirms all discrepancies resolved.
...
- Python manual SRMR bug identified (SRMR ~.129). Lavaan SRMR = .026 (CFA) / .024 (SEM). Bootstrap CIs added; fabricated CIs removed; documented in `FACT_CHECK_AUDIT.md` D1–D3.
